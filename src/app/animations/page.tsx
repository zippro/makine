                        ...
                        {visibleFolders.map(folderPath => {
                            ...
                            return (
                                <div
                                    key={folderPath}
                                    onDoubleClick={() => setCurrentFolder(folderPath)}
                                    className="group relative rounded-2xl border border-white/10 bg-white/[0.02] p-6 flex flex-col items-center justify-center gap-4 hover:bg-white/[0.05] hover:border-white/20 cursor-pointer transition-all aspect-video backdrop-blur-sm"
                                >
                                    <div className="p-4 rounded-full bg-white/5 text-gray-400 group-hover:scale-110 group-hover:bg-white/10 group-hover:text-white transition-all duration-300">
                                        <ChevronDown className="w-8 h-8" />
                                    </div>
                                    <div className="text-center">
                                        <span className="block font-medium text-gray-200 tracking-wide">{folderName}</span>
                                        <span className="text-xs text-gray-500 mt-1">{count} animation{count !== 1 ? 's' : ''}</span>
                                    </div>
                                </div>
                            )
                        })}
